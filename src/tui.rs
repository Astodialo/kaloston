...
impl App {
    ...
    pub async fn move_cursor_left(&mut self) {
        let cursor_moved_left = self.character_index.saturating_sub(1);
        self.character_index = self.clamp_cursor(cursor_moved_left).await;
    }

    pub async fn move_cursor_right(&mut self) {
        let cursor_moved_right = self.character_index.saturating_add(1);
        self.character_index = self.clamp_cursor(cursor_moved_right).await;
    }
    ...
    pub async fn byte_index(&self) -> usize {
        self.input
            .char_indices()
            .map(|(i, _)| i)
    ...
    pub async fn enter_char(&mut self, new_char: char) {
    ...
    pub async fn delete_char(&mut self) {
        ...
        if is_not_cursor_leftmost {
            ...
            // Getting all characyers after the selected character.
    ...
    pub async fn clamp_cursor(&self, new_cursor_pos: usize) -> usize {
        new_cursor_pos.clamp(0, self.input.chars().count())
    }

    pub async fn reset_cursor(&mut self) {
